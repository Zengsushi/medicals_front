...
const getIcon = (iconName) => {
  ...
  const PascalCaseName = iconName.replace(/-([a-z])/g, (_, letter) => letter.toUpperCase());
...
const availablePositionOptions = computed(() => {
  if (formData.value.parent_id) {
    // 有父菜单时，查找父菜单的位置
    const parentMenu = findMenuById(formData.value.parent_id, menuData.value);
    if (parentMenu) {
      // 根据父菜单的位置返回对应的子菜单位置
      if (parentMenu.position === 'top') {
        return [
          { value: 'top_child', label: '顶部子菜单' }
        ];
      } else if (parentMenu.position === 'left') {
        return [
          { value: 'left_child', label: '左侧子菜单' }
        ];
      }
    }
    // 默认子菜单位置
...
// 监听父菜单变化，自动设置位置
watch(() => formData.value.parent_id, (newParentId, oldParentId) => {
  if (newParentId && newParentId !== oldParentId) {
    // 有父菜单时，根据父菜单的位置设置子菜单的位置
    const parentMenu = findMenuById(newParentId, menuData.value);
    if (parentMenu) {
      if (parentMenu.position === 'top') {
        formData.value.position = 'top_child';
      } else if (parentMenu.position === 'left') {
        formData.value.position = 'left_child';
      }
    }
  } else if (!newParentId) {
    // 没有父菜单时，设置为默认的顶级菜单位置
    if (formData.value.position === 'top_child') {
      formData.value.position = 'top';
    } else if (formData.value.position === 'left_child') {
      formData.value.position = 'left';
    }
  }
});
...
const handleAddChild = (menu) => {
  editingMenu.value = null;
  // 自动继承父菜单的位置
  let childPosition = 'top_child';
  if (menu.position === 'top') {
    childPosition = 'top_child';
  } else if (menu.position === 'left') {
    childPosition = 'left_child';
  } else if (menu.position === 'top_child') {
    childPosition = 'top_child';
  } else if (menu.position === 'left_child') {
    childPosition = 'left_child';
  }
